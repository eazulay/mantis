...
<?php
	...
	$t_category = gpc_get_string( 'show_category', '' ); // Added by Eyal Azulay
...
<?php
	if ( !is_blank( $f_get_bugnote_stats_button ) ) {
		...
		$t_bugnote_stats = bugnote_stats_get_project_array( $f_project_id, $t_from, $t_to, $f_bugnote_cost, $t_category );
...
<?php
		...
		foreach ( $t_bugnote_stats as $t_item ) {
			...
			if ( $t_item['bug_id'] != $t_prev_id) {
				$t_link = $t_item['project_name'] . " " . string_get_bug_view_link( $t_item['bug_id'] ) . ": " . string_display( $t_item['summary'] );
