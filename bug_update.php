	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		if ( !$f_update_mode ){
			if ( !( ( $t_def['require_' . $t_custom_status_label] ) ||
					( $t_def['display_' . $t_custom_status_label] && in_array( $t_custom_status_label, array( "resolved", "closed" ) ) ) ||
					( $t_id == 2 ) || // Always update Info Require From, so it is cleared when Status is changed not to Info Required
//					( $t_bug_data->status == $t_feedback && $t_id == 2 ) || // Uncomment this line instead of one above if clearing Info Require From is not desired
					( ($t_bug_data->status == CONFIRMED || $t_bug_data->status == HOLD) && $t_id >= 3 && $t_id < 8) ) ) {
				continue;
			}
		}
		...
		if( !gpc_isset_custom_field( $t_id, $t_def['type'] ) && ( $t_def['require_' . $t_custom_status_label] ) ) {
	...
	if ( false == $t_bug_note_set ) {
		$note_id = bugnote_add( $f_bug_id, $f_bugnote_text, $f_time_tracking, $f_private, 0, '', NULL, FALSE );
		if ( $t_bug_data->status == CONFIRMED && $note_id > 0 )
			custom_field_set_value( 8, $f_bug_id, $note_id );
	}
