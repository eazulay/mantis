...
class BugData {
	...
	function validate( $p_update_extended =  true) {
		...
		if ($this->category_id == 17 && $this->reproducibility == 100 ) { // Bug and reproducibility is "(not a bug)"
	...
	function create() {
		...
		# Add Administrators as Monitors (added by Eyal Azulay)
		$user_result = users_by_level( ADMINISTRATOR );
		$t_count = db_num_rows( $user_result );
		for( $i = 0; $i < $t_count; $i++ ) {
			$t_user_id = db_result( $user_result, $i );
			if ( $this->reporter_id != $t_user_id )
				bug_monitor( $this->id, $t_user_id );
		}
...
function bug_close( $p_bug_id, $p_bugnote_text = '', $p_bugnote_private = false, $p_time_tracking = '0:00' ) {
	...
	bug_set_field( $p_bug_id, 'priority', NONE );
