 ...
 * @return array
 */
function email_collect_recipients( $p_bug_id, $p_notify_type, $p_extra_user_ids_to_email = array(), $users_to_skip = array() ) {
	...
	foreach( $t_recipients as $t_id => $t_ignore ) {
		...
		# If user was included in To or CC and should be skipped
		if( in_array( $t_id, $users_to_skip ) ){
			log_event( LOG_EMAIL_RECIPIENT, sprintf( 'Issue = #%d, drop @U%d (included in trigger email)', $p_bug_id, $t_id ) );
			continue;
		}
		...
		if( is_blank( $t_email ) ) {
			...
			# @@@ we could check the emails for validity again but I think it would be too slow
...
function email_generic( $p_bug_id, $p_notify_type, $p_message_id = null, $p_header_optional_params = null, $p_extra_user_ids_to_email = array() ) {
	...
	if( ON === config_get( 'enable_email_notification' ) ) {
		...
		# Check if email address was included in To or CC
		$latest_note_text = "";
		if (count( $t_all_bugnotes ) > 0)
			$latest_note_text = end($t_all_bugnotes)->note;
		$users_to_skip = array();
		$text_line1 = strtok($latest_note_text, "\n");
		if (substr($text_line1, 0, 11) == "Email from:"){
			$text_line2 = strtok("\n");
			$text_line3 = strtok("\n");
			if (substr($text_line2, 0, 4) == "To: "){
				$email = strtok(substr($text_line2, 4), ";");
				while ($email !== false){
					$users_to_skip[] = user_get_id_by_email(rtrim($email));
					$email = strtok(";");
				}
			}
			if (substr($text_line3, 0, 4) == "CC: "){
				$email = strtok(substr($text_line3, 4), ";");
				while ($email !== false){
					$users_to_skip[] = user_get_id_by_email(rtrim($email));
					$email = strtok(";");
				}
		...
		# @todo yarick123: email_collect_recipients() will be completely rewritten to provide additional information such as language, user access,..
		# @todo yarick123:sort recipients list by language to reduce switches between different languages
		$t_recipients = email_collect_recipients( $p_bug_id, $p_notify_type, $p_extra_user_ids_to_email, $users_to_skip );
...
function email_bugnote_add( $p_bug_id ) {
	email_generic( $p_bug_id, 'bugnote', 'email_notification_title_for_action_bugnote_submitted', array('') );
}
...
function email_send( $p_email_data ) {
	...
	$mail->WordWrap = 110; // 80;        # set word wrap to 80 characters
	...
	if ($reporting_mailboxes !== false && isset($t_email_data->metadata['project_id'])){
		...
		foreach( $reporting_mailboxes as $mailbox ){
			...
			if ($project_id == $mailbox['project_id']){
				$mail->From = $mailbox['erp_username'];
				break;
			}
		}
	}
...
function email_bug_reminder( $p_recipients, $p_bug_id, $p_message ) {
	...
	foreach( $p_recipients as $t_recipient ) {
		...
		if( ON == config_get( 'enable_email_notification' ) ) {
			email_store( $t_email, $t_subject, $t_contents, null, $t_project_id );
		}
...
function email_bug_info_to_one_user( $p_visible_bug_data, $p_message_id, $p_project_id, $p_user_id, $p_header_optional_params = null ) {
	...
	date_default_timezone_set( user_pref_get_pref( $p_user_id, 'timezone' ) );
	
	# build subject  // removed call to bug_format_id()
	$t_subject = $p_visible_bug_data['email_project'] . ' ' . $p_visible_bug_data['email_bug'] . ': ' . $p_visible_bug_data['email_summary'];
...
function email_format_bug_message( $p_visible_bug_data ) {
	...
	// Assigned
	if( $t_status == ASSIGNED && !empty( $p_visible_bug_data['email_handler'] ) )
		$t_message .= ' to ' . $p_visible_bug_data['email_handler'];
	$t_message .= "\n";
	...
	//$t_message .= email_format_attribute( $p_visible_bug_data, 'email_project', false );
...
/*	foreach( $p_visible_bug_data['custom_fields'] as $t_custom_field_name => $t_custom_field_data ) {
	...
	if( config_get( 'bug_resolved_status_threshold' ) <= $t_status ) {
		...
		if( config_get( 'bug_resolved_status_threshold' ) == $t_status && $p_visible_bug_data['email_resolution'] == 'fixed' )
			$t_message .= "Please test and confirm User Acceptance by reply to this email, or Change Status To: Closed in Mantis.\nAlternatively, if your test fails, explain why and Change Status To: Failed UAT in Mantis.";
	}
	//$t_message .= $t_email_separator2 . " \n";
...
/* Eyal commented this block out for issue #194
...
*/
/* Eyal commented this block out for issue #106 ~2526
...
*/
	...
	$CONST_BUGNOTE_LIMIT = 3;
	$bugnote_limit = $CONST_BUGNOTE_LIMIT;
	$first_note = true;
	
	# format bugnotes
	foreach( $p_visible_bug_data['bugnotes'] as $t_bugnote ) {
		$t_date_submitted = date( $t_normal_date_format, $t_bugnote->date_submitted );

		//$t_formatted_bugnote_id = bugnote_format_id( $t_bugnote->id );
		$t_formatted_bugnote_id = $t_bugnote->id;
		...
		if ( $first_note && $p_visible_bug_data['notify_type'] == 'bugnote' )
			$t_message .= 'NEW ';
		
		if ( $t_bugnote->view_state == VS_PRIVATE )
			$t_message .= 'PRIVATE ';

		$t_message .= 'Note ' . $t_formatted_bugnote_id . ' by ' . user_get_name( $t_bugnote->reporter_id ) . ' on ' . $t_date_submitted . ($first_note ? ' (your time)' : '');
		$t_message .= "\n\n" . $t_bugnote->note . "\n\n" . $t_bugnote_link . "\n";
	...
	$notes_remaining = count($p_visible_bug_data['bugnotes']) - $CONST_BUGNOTE_LIMIT;
	if ($notes_remaining > 0)
		$t_message .= "This Issue also contains $notes_remaining older notes.\n";
	
	if( isset( $p_visible_bug_data['email_bug_view_url'] ) )
		$t_message .= "Go to Issue: " . $p_visible_bug_data['email_bug_view_url'] . "\n";
 ...
 * @return string
 */
function email_format_attribute( $p_visible_bug_data, $attribute_id, $new_line = true ) {
	if( array_key_exists( $attribute_id, $p_visible_bug_data ) ) {
		return utf8_str_pad( lang_get( $attribute_id ) . ': ', config_get( 'email_padding_length' ), ' ', STR_PAD_RIGHT ) . ( $new_line ? $p_visible_bug_data[$attribute_id] . "\n" : utf8_str_pad( $p_visible_bug_data[$attribute_id], config_get( 'email_padding_length' ), ' ', STR_PAD_RIGHT ) );
	}
	return '';
}
