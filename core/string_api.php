...
function string_get_bug_view_link( $p_bug_id, $p_user_id = null, $p_detail_info = true, $p_fqdn = false ) {
	if( bug_exists( $p_bug_id ) ) {
		...
		$t_link .= '>#' . $p_bug_id . '</a>'; // Eyal: Issue 1120 changed from following line
...
function string_get_bugnote_view_link( $p_bug_id, $p_bugnote_id, $p_user_id = null, $p_detail_info = true, $p_fqdn = false ) {
	...
	if( bug_exists( $t_bug_id ) && bugnote_exists( $p_bugnote_id ) ) {
		...
		if( $p_detail_info ) {
			...
			$t_link .= ' title="' . bug_format_id( $t_bug_id ) . ': [' . $t_update_date . '] ' . $t_reporter . (  $t_time_tracking > 0 && access_has_bug_level( config_get( 'time_tracking_view_threshold' ), $t_bug_id ) ? " (".lang_get('time_tracking_time_spent').' '.db_minutes_to_hhmm($t_time_tracking).")" : '' ) . '"';
		}
