...
function users_by_level( $p_level = ANYBODY ) {
	$t_level = db_prepare_int( $p_level );
	$t_user_table = db_get_table( 'mantis_user_table' );
	$query = "SELECT id FROM $t_user_table WHERE access_level>=" . db_param();
	$t_users = db_query_bound( $query, Array( $t_level ) );
...
function user_get_id_by_email( $p_email ) {
	...
	$query = "SELECT *
			  ...
			  OR FIND_IN_SET(" . db_param() . ", additional_email_addresses)"; // added by Eyal Azulay
	$result = db_query_bound( $query, Array( $p_email, $p_email ) );
