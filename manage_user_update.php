...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	$f_additional_emails = gpc_get_string( 'additional_emails', '' );
	...
	if ( $f_additional_emails != "") {
		$t_additional_emails = email_append_domain( trim( $f_additional_emails ) );
		email_ensure_valid( $t_additional_emails );
		email_ensure_not_disposable( $t_additional_emails );
		$c_additional_emails = $t_additional_emails;
	}
	...
	if ( $f_protected && $t_old_protected ) {
	    $query = "UPDATE $t_user_table
	    		SET username=" . db_param() . ", email=" . db_param() . ",
					...
					additional_email_addresses=" . db_param() . "
	    		WHERE id=" . db_param();
	    $query_params = Array( $c_username, $c_email, $c_protected, $c_realname, $c_additional_emails, $c_user_id );
	} else {
	    $query = "UPDATE $t_user_table
	    		SET username=" . db_param() . ", email=" . db_param() . ",
					...
					additional_email_addresses=" . db_param() . "
	    		WHERE id=" . db_param();
	    $query_params = Array( $c_username, $c_email, $c_access_level, $c_enabled, $c_protected, $c_realname, $c_additional_emails, $c_user_id );
	}
