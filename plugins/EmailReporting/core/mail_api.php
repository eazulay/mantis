...
class ERP_mailbox_api
{
	...
	private function process_single_email( $p_i, $p_overwrite_project_id = FALSE )
	{
		...
		if ( $t_email[ 'Reporter_id' ] !== FALSE )
		{
			...
			{
				if ($t_email[ 'From_parsed' ][ 'email' ]!='mantis@get-it-write.com')
					$this->add_bug( $t_email, $p_overwrite_project_id );
				else
					$this->custom_error( 'Email from Mantis rejected: ' . $t_email[ 'From_parsed' ][ 'email' ] );
			}
			else
			{
				$this->custom_error( 'From email address rejected by email_is_valid function based on: ' . $t_email[ 'From_parsed' ][ 'email' ] );
			}
		}
...
/*			foreach( $t_related_custom_field_ids as $t_id )
{
	...
	private function get_bug_id_from_subject( $p_mail_subject )
	{
		...
		{
			case 'balanced':
				$t_subject_id_regex = "/(RE|FW|FWD|):?(?P<project>[^0-9]+)\s0*(?P<id>[0-9]+):/iu";
				// "/\[(?P<project>[^\]]+\s|)0*(?P<id>[0-9]+)\]/u";
				break;

			case 'relaxed':
				$t_subject_id_regex = "/(RE|FW|FWD|):?(?P<project>[^0-9]+)\s0*(?P<id>[0-9]+)/iu";
				// "/\[(?P<project>[^\]]*\s|)0*(?P<id>[0-9]+)\s*\]/u";
				break;

			case 'strict':
			default:
				$t_subject_id_regex = "/(RE|FW|FWD|):?(?P<project>[^0-9]+)\s0*(?P<id>[0-9]+):\s?/iu";
				// "/\[(?P<project>[^\]]+\s)0*(?P<id>[0-9]+)\]/u";
		}
	...
	private function add_additional_info( $p_type, &$p_email, $p_description )
	{
		...
		if ( is_array($p_email[ 'To' ]) ){
			$t_additional_info .= 'To: ' . implode(';', $p_email[ 'To' ]) . "\n";
		}
		if ( is_array($p_email[ 'Cc' ]) ){
			$t_additional_info .= 'CC: ' . implode(';', $p_email[ 'Cc' ]) . "\n";
		}
