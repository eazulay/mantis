 ...
 * @access public
 */
function column_billing_get_title( $p_column ) {
	$t_custom_field = column_get_custom_field_name( $p_column );
	if( $t_custom_field !== null ) {
		$t_field_id = custom_field_get_id_from_name( $t_custom_field );
	...
	switch( $p_column ) {
		case 'username':
			return plugin_lang_get( 'username_csv' );
		case 'user_price':
			return plugin_lang_get( 'hourly_rate_csv' );
		case 'summary':
			return plugin_lang_get( 'desc_csv' );
		case 'bug_id':
			return plugin_lang_get( 'bug_csv' );
		case 'sum_time_tracking':
			return plugin_lang_get( 'hours_csv' );
		case 'total_amt':
			return plugin_lang_get( 'total_csv' );
		default:
			return lang_get_defaulted( $p_column );
	}
}

function column_billing_get_field($p_key, $p_field){
	switch ($p_key){
	    case 'user_price': return $p_field;
	    default: return csv_escape_string($p_field);
	}	
	return csv_escape_string($p_field);
}

function column_billing_get_date($p_date){
    if($p_date){
        $t_parts = split("-", $p_date);
        if (is_array($t_parts)){
            return (strlen($t_parts[1])==1 ? "0" . $t_parts[1] : $t_parts[1]) . "-" . (strlen($t_parts[2])==1 ? "0" . $t_parts[2] : $t_parts[2]) . "-" . $t_parts[0];
        }
    }else{
        return;
    }	
