...
function html_meta_redirect( $p_url, $p_time = null, $p_sanitize = true ) {
	...
	if(plugin_get_current()=="TimeTrackingPatch"){
		$t_url = htmlspecialchars_decode( $t_url );
	}
...
function print_menu() {
	if( auth_is_user_authenticated() ) {
		...
		if( config_get( 'time_tracking_enabled' ) && config_get( 'time_tracking_with_billing' ) && access_has_global_level( config_get( 'time_tracking_reporting_threshold' ) ) && plugin_is_installed("TimeTrackingPatch")) {
	        $t_menu_options[] = '<a href="' . helper_mantis_url( 'plugin.php?page=TimeTrackingPatch/billing_page.php">' ) . lang_get( 'time_tracking_billing_link' ) . '</a>';
		}
...
function print_manage_menu( $p_page = '' ) {
	$t_manage_user_page = 'plugin.php?page=TimeTrackingPatch/manage_user_page.php';
	$t_manage_project_menu_page = 'plugin.php?page=TimeTrackingPatch/manage_proj_page.php';
...
function html_button( $p_action, $p_button_text, $p_fields = null, $p_method = 'post' ) {
	...
	if(plugin_get_current()!="TimeTrackingPatch"){
		...
		$p_action = 'plugin.php?page='.plugin_get_current().'/'.$p_action;
	}
