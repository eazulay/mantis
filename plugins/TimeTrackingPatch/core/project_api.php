...
function project_create( $p_name, $p_description, $p_status, $p_view_state = VS_PUBLIC, $p_file_path = '', $p_enabled = true, $p_inherit_global = true, $p_user_price=0 ) {

	$c_enabled = db_prepare_bool( $p_enabled );
	$c_inherit_global = db_prepare_bool( $p_inherit_global );
	$c_user_price = db_prepare_int( $p_user_price );
	...
	$query = "INSERT INTO $t_project_table
					( name, status, enabled, view_state, file_path, description, inherit_global, user_price )
				  VALUES
					( " . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() .')';

	db_query_bound( $query, Array( $p_name, (int) $p_status, $c_enabled, (int) $p_view_state, $p_file_path, $p_description, $c_inherit_global, $c_user_price ) );
...
# --------------------
# Update a project
function project_update( $p_project_id, $p_name, $p_description, $p_status, $p_view_state, $p_file_path, $p_enabled, $p_inherit_global, $p_user_price ) {

	$p_project_id = (int) $p_project_id;
	$c_enabled = db_prepare_bool( $p_enabled );
	$c_inherit_global = db_prepare_bool( $p_inherit_global );
	$c_user_price = db_prepare_string( $p_user_price );
	...
	$query = "UPDATE $t_project_table
				  SET name=" . db_param() . ",
					...
					user_price=" . db_param() . "
				  WHERE id=" . db_param();
	db_query_bound( $query, Array( $p_name, (int) $p_status, $c_enabled, (int) $p_view_state, $p_file_path, $p_description, $c_inherit_global, $c_user_price, $p_project_id ) );
...
# return the descriptor holding all the info from the project user list
# for the specified project
function project_get_local_user_row( $p_project_id, $p_user_id ) {
	$t_project_user_list_table = db_get_table( 'mantis_project_user_list_table' );
...
# ===================================
# add user with the specified access level to a project
function project_change_dev_price( $p_project_id, $p_price, $p_user_id, $p_access_level ) {
	$t_project_user_list_table = db_get_table( 'mantis_project_user_list_table' );
