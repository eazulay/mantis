...
//require_once( 'timetracking_bugnote_api.php' );
...
<?php
	...
	$f_bugnote_price = gpc_get_string( 'bugnote_price_check', '' );
	...
	if ( ON == plugin_config_get( 'time_tracking_with_pricing' )) {
		$t_price_chk = true;
	} else {
		$t_price_chk = false;
	}
...
?>
<form method="post" action="<?php echo plugin_page( 'billing_page.php' ); ?>">
...
if ( !is_blank( $f_get_bugnote_stats_button ) ) {
	...
	if($t_price_col && ALL_PROJECTS == $f_project_id && !$f_bugnote_cost){?>
			<table border="0" class="width100" cellspacing="0">
			<tr class="row-1">
			<td><?php echo plugin_lang_get('project_required');?></td>
			</tr>
			</table>
<?php }else{
...
<?php }else{ ?>
    <td>
		<?php echo string_attribute(number_format( ($t_item['sum_time_tracking']* $t_item['user_price'])/60, 2 )); ?>
	</td>
<?php }}else{?>
<td></td>
<?php }?>
...
<input type="hidden" name="d_to" value="<?php print $t_to;?>">
<input type="hidden" name="project_id" value="<?php print $f_project_id;?>">
<input type="hidden" name="bugnote_cost" value="<?php print $f_bugnote_cost;?>">
