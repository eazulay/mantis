...
<!-- PROJECT PROPERTIES -->
<div align="center">
<form method="post" action="<?php print plugin_page('manage_proj_update.php', true);?>">
...
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
		<?php echo plugin_lang_get( 'project_price_per_hour_lbl' ) ?>
	</td>
	<td>
		<input type="text" name="user_price" value="<?php echo string_attribute( $row['user_price'] ) ?>" />
		<small><?php echo plugin_lang_get('max_roundoff_hint');?></small>
	</td>
</tr>
...
<div class="border center">
	<form method="post" action="<?php print plugin_page('manage_proj_delete.php', true)?>">
	...
	<td class="form-title" colspan="6">
		<?php 
			...
			if ( access_has_global_level ( config_get( 'create_project_threshold' ) ) ) {
				print_button( plugin_page('manage_proj_create_page.php&parent_id=' . $f_project_id, true), lang_get( 'create_new_subproject_link' ) );
			}
		?>
	</td>
</tr>
...
<!-- Subprojects -->
<form name="update_children_form" action="<?php print plugin_page('manage_proj_update_children.php', true)?>" method="post">
...
<tr <?php echo helper_alternate_class() ?>>
	<td>
		<a href="<?php print plugin_page("manage_proj_edit_page.php&project_id=".$t_subproject['id'], true); ?>"><?php echo string_display( $t_subproject['name'] ) ?></a>
	</td>
	<td class="center">
	...
	<td class="center">
		<?php
				print_bracket_link( plugin_page('manage_proj_edit_page.php&project_id=' . $t_subproject['id'], true), lang_get( 'edit_link' ) );
				print_bracket_link( plugin_page("manage_proj_subproj_delete.php&project_id=$f_project_id&subproject_id=" . $t_subproject['id'] . form_security_param( 'manage_proj_subproj_delete' ), true), lang_get( 'unlink_link' ) );
		?>
	</td>
</tr>
<?php
...
<tr>
	<td colspan="7">
		<form method="post" action="<?php print plugin_page('manage_proj_subproj_add.php', true);?>">
		...
		<tr <?php echo helper_alternate_class() ?>>
			...
			<td class="center">
				<?php if ( !$t_inherited ) {
					...
					print_button( plugin_page('manage_proj_cat_edit_page.php&id=' . $t_id . '&project_id=' . $t_project_id, true), lang_get( 'edit_link' ) );
					echo '&nbsp;';
					print_button( plugin_page('manage_proj_cat_delete.php&id=' . $t_id . '&project_id=' . $t_project_id, true), lang_get( 'delete_link' ) );
				} ?>
			</td>
		</tr>
<?php
...
<tr>
	<td class="left" colspan="3">
		<form method="post" action="<?php print plugin_page('manage_proj_cat_add.php', true);?>">
...
<tr>
	<td class="left" colspan="3">
		<form method="post" action="<?php print plugin_page('manage_proj_cat_copy.php', true);?>">
		...
		<tr <?php echo helper_alternate_class() ?>>
			...
			<td class="center">
				<?php
					...
					if ( !$t_inherited ) {
						print_button( plugin_page('manage_proj_ver_edit_page.php&version_id=' . $t_version_id, true), lang_get( 'edit_link' ) );
						echo '&nbsp;';
						print_button( plugin_page('manage_proj_ver_delete.php&version_id=' . $t_version_id, true), lang_get( 'delete_link' ) );
					}
				?>
			</td>
		</tr>
...
<tr>
	<td class="left" colspan="3">
		<form method="post" action="<?php print plugin_page('manage_proj_ver_add.php', true)?>">
...
<tr>
	<td class="left" colspan="3">
		<form method="post" action="<?php print plugin_page('manage_proj_ver_copy.php', true);?>">
...
?>
	...
	?>
			<tr <?php echo helper_alternate_class() ?>>
				...
				<td>
<form method="post" action="<?php print plugin_page('manage_proj_custom_field_update.php', true);?>">
	...
	?>
				...
				<?php
					...
					print_button( plugin_page("manage_proj_custom_field_remove.php&field_id=$t_field_id&project_id=$f_project_id", true), lang_get( 'remove_link' ) );
				?>
				</td>
			</tr>
	<?php
...
?>
	<tr>
		<td class="left" colspan="3">
			<form method="post" action="<?php print plugin_page('manage_proj_custom_field_add_existing.php', true);?>">
	...
	<tr>
		<td class="left" colspan="3">
			<form method="post" action="<?php print plugin_page('manage_proj_custom_field_copy.php', true);?>">
...
<div align="center">
	<table class="width75" cellspacing="1">
		<form method="post" action="<?php print plugin_page('manage_proj_user_add.php');?>">
		...
		<!-- Copy Users Form -->
		<form method="post" action="<?php print plugin_page('manage_proj_user_copy.php', true);?>">
...
?>
		<tr <?php echo helper_alternate_class() ?>>
			...
			<?php
				...
				if ( $t_can_manage_users && access_has_project_level( $t_user['access_level'], $f_project_id ) ) {
					if ( project_includes_user( $f_project_id, $t_user['id'] )  ) {
						print_button( plugin_page("manage_proj_user_remove.php&project_id=$f_project_id&user_id=" . $t_user['id']), lang_get( 'remove_link' ) );
			...
			<td class="center">
			    <form method="post" action="<?php print plugin_page('manage_proj_change_dev_price.php');?>">
			        <?php echo form_security_field( 'manage_proj_dev_price_change' ) ?>
			        <input type="hidden" name="project_id" value="<?php echo $f_project_id ?>" />
			        <input type="hidden" name="access_level" value="<?php echo $t_user['access_level'] ?>" />
			        <input type="hidden" name="user_id" value="<?php echo $t_user['id'] ?>" />
			        <?php $f_pro_user_row =  project_get_local_user_row($f_project_id, $t_user['id'] );?>
			        <input type="text" name="user_price" value="<?php echo ($f_pro_user_row['user_price']);?>" />
			        
			        <input type="submit" name="change" class="button" value="<?php echo plugin_lang_get( 'change_lbl' ) ?>" />
			    </form>    
			</td>
		</tr>
<?php
...
?>
	...
	<?php
		...
		if ( !$f_show_global_users ) {
			print_button( plugin_page("manage_proj_edit_page.php&project_id=$f_project_id&show_global_users=true", true), lang_get( 'show_global_users' ) );
		} else {
			print_button( plugin_page("manage_proj_edit_page.php&project_id=$f_project_id", true), lang_get( 'hide_global_users' ) );
		}

		if ( $t_removable_users_exist ) {
			echo '&nbsp;';
			print_button( plugin_page("manage_proj_user_remove.php&project_id=$f_project_id", true), lang_get( 'remove_all_link' ) );
		}
	?>
	</td>
	</tr>
