	...
	<td class="form-title" colspan="5">
		<?php 
			...
			if ( access_has_global_level ( config_get( 'create_project_threshold' ) ) ) {
				print_button( plugin_page('manage_proj_create_page.php'), lang_get( 'create_new_project_link' ) );
			}
		?>
	</td>
</tr>
<tr class="row-category">
	<td width="20%">
		<?php 
			print_manage_project_sort_link( plugin_page('manage_proj_page.php'), lang_get( 'name' ), 'name', $t_direction, $f_sort );
			print_sort_icon( $t_direction, $f_sort, 'name' );
		?>
	</td>
	<td width="10%">
		<?php 
			print_manage_project_sort_link( plugin_page('manage_proj_page.php'), lang_get( 'status' ), 'status', $t_direction, $f_sort );
			print_sort_icon( $t_direction, $f_sort, 'status' );
		?>
	</td>
	<td width="10%">
		<?php 
			print_manage_project_sort_link( plugin_page('manage_proj_page.php'), lang_get( 'enabled' ), 'enabled', $t_direction, $f_sort );
			print_sort_icon( $t_direction, $f_sort, 'enabled' );
		?>
	</td>
	<td width="10%">
		<?php 
			print_manage_project_sort_link( plugin_page('manage_proj_page.php'), lang_get( 'view_status' ), 'view_state', $t_direction, $f_sort );
			print_sort_icon( $t_direction, $f_sort, 'view_state' );
		?>
	</td>
	<td width="40%">
		<?php 
			print_manage_project_sort_link( plugin_page('manage_proj_page.php'), lang_get( 'description' ), 'description', $t_direction, $f_sort );
...
<tr <?php echo helper_alternate_class() ?>>
	<td>
		...
		<a href="<?php print plugin_page('manage_proj_edit_page.php&project_id='. $t_project['id']); ?>"><?php echo str_repeat( "&raquo; ", $t_level ) . string_display( $t_project['name'] ) ?></a>
	</td>
	<td>
