...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	foreach ( $t_prefix_array as $t_prefix => $t_caption ) {
		...
		if ( $t_prefix === $f_filter ) {
			...
			print_link( "plugin.php?page=".plugin_get_current()."/manage_user_page.php?sort=$c_sort&dir=$c_dir&save=1$t_hide_filter&filter=$t_prefix", $t_caption );
		}
...
<tr>
	<td class="form-title" colspan="5">
		...
		<?php print_button( plugin_page('manage_user_create_page.php'), lang_get( 'create_new_account_link' ) ) ?>
		<?php if ( $f_filter === 'UNUSED' ) echo print_button( 'manage_user_prune.php', lang_get( 'prune_accounts' ) ); ?>
	</td>
	<td class="center" colspan="3">
		<form method="post" action="<?php plugin_page('manage_user_page.php')?>">
...
<tr class="row-category">
	<td>
		<?php
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'username' ), 'username', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'username' );
		?>
	</td>
	<td>
		<?php
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'realname' ), 'realname', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'realname' );
		?>
	</td>
	<td>
		<?php
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'email' ), 'email', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'email' );
		?>
	</td>
	<td>
		<?php 
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'access_level' ), 'access_level', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'access_level' );
		?>
	</td>
	<td>
		<?php
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'enabled' ), 'enabled', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'enabled' );
		?>
	</td>
	<td>
		<?php 
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), $t_lock_image, 'protected', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'protected' );
		?>
	</td>
	<td>
		<?php 
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'date_created' ), 'date_created', $c_dir, $c_sort, $c_hide, $c_filter );
			print_sort_icon( $c_dir, $c_sort, 'date_created' );
		?>
	</td>
	<td>
		<?php 
			print_manage_user_sort_link(  plugin_page('manage_user_page.php'), lang_get( 'last_visit' ), 'last_visit', $c_dir, $c_sort, $c_hide, $c_filter );
...
<tr <?php echo helper_alternate_class( $i ) ?>>
	...
	?>
		<a href="<?php echo plugin_page( 'manage_user_edit_page.php&user_id='.$u_id ); ?>"><?php echo string_display_line( $u_username ) ?></a>
	<?php
		} else {
...
?>
	<br />
	<form method="post" action="<?php print plugin_page('manage_user_edit_page.php');?>">
	<?php # CSRF protection not required here - form does not result in modifications ?>
